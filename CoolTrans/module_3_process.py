...
def steel1_read_data_from_output_2(in_file_path):
    ...
    with open(in_file_path,"r") as f:
        ...
        for index,char in enumerate(line_list):                    
            if 'SI' in char:
                SI = line_list[index+1]
            if 'C' in char:
                C = line_list[index+1]
            if 'MN' in char:
                MN = line_list[index+1]
        return [SI,C,MN]
...
def ferrite_solubility(input_dict,module_3_model_steel1_dir,module_3_run_dir,out_file_dir):
    input_dict['C_wt'] = str(float(input_dict['C_wt'])/100)
    input_dict['Mn_wt'] = str(float(input_dict['Mn_wt'])/100)
    input_dict['Si_wt'] = str(float(input_dict['Si_wt'])/100)
    in_file = os.path.join(module_3_model_steel1_dir,'Para_calc.tcm')
    out_file_path = out_file_dir 
    out_file_dir = os.path.join(module_3_run_dir,out_file_path)
    #os.mkdir(out_file_dir)
    out_file = os.path.join(out_file_dir,'Para_calc.tcm')
    ...
    for i in range(80):
        ...
        if 'output_1.txt' in os.listdir(out_file_dir):
            time.sleep(5)
            dead_wait = 0
            break
    if dead_wait == 1:
    ...
    txt1_file = os.path.join(out_file_dir,'output_1.txt')
    r=steel1_read_data_from_output_1(txt1_file)
    input_dict['T1'],input_dict['T0'],input_dict['T2'],input_dict['X0'] = str(r[0]),str(r[2]),str(r[1]),str(r[1])
    pids = psutil.pids()
    taskkill()
    time.sleep(5)
...
def calcu_fsc(input_dict,module_3_model_steel1_dir,module_3_run_dir,out_file_dir):
    ...
    if float(input_dict['C_wt']) >= float(input_dict['X0']):
        input_dict['output'] = 'ferrite_fraction.txt'
        model_path = os.path.join(module_3_model_steel1_dir,'ferrite_fraction.dcm')
        
        ferrite_fraction = float(read_output(run_dcm(input_dict,model_path,run_path))) * 6E6 * 100 / float(input_dict['grain_size'])
        ...
        #print(read_output(run_dcm(input_dict,model_path,run_path)),0)
        cementite_fraction = float(read_output(run_dcm(input_dict,model_path,run_path))) *  6E6 * 100 / float(input_dict['grain_size'])
        ferrite_fraction = 0 
