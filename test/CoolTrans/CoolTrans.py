...
class COOTRANS(object):
    ...
    def calculate_V(self,position):
        print("T",position['T'][0],len(position['T'][0]))
        print("t",position['t'][0])
        begin = 0
        for index in range(1,len(position['T'][0])-2):
            print("-",index)
            if position['T'][0][index] < position['T'][0][index+1]:
                print("-"*10)
                print(position['T'][0][index] - position['T'][0][index+1])
                break
        end = index
        print(index,begin,abs(int((position['T'][0][end] - position['T'][0][begin])/(position['t'][0][end] - position['t'][0][begin]))))
        print(position['T'][0][index],position['T'][0][index+1])
        raise Exception("V")
