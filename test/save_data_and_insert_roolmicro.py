...
if len(sys.argv) > 1:
    # grain_growth src id
    task_id = '4_1567495888692_00'
    mongodb = exceptProcess.saferun(utils.Mongo,[],'other')
    data = mongodb.query_taskid(task_id)
    data.pop('_id')
    for key in data:
        print(key)
    data['task_id'] = '0_wangxin_test_rollmicro_05'
    mongodb.simulationInput.insert_one(data) #input sheet
    #temp
    temp_id = '4_1567495593064_00'
    mongo = utils.Mongo(simulationInput='SimulationOutputData')
    data = mongo.query_taskid(temp_id)
    data.pop('_id')
    data['task_id'] = '0_wangxin_test_rollmicro_04'
    mongodb.out_insert_dict(data) #out sheet
    data['task_id'] = '0_wangxin_test_rollmicro_03'
    data.pop('_id')
    mongodb.out_insert_dict(data) # out sheet
    print("cooltrans")

    temp_id = '0_1567475483899_02'
    mongo = utils.Mongo(simulationInput='SimulationOutputData')
    data = mongo.query_taskid(temp_id)
    data.pop('_id')
    data['task_id'] = '0_wangxin_test_rollmicro_02'
    mongodb.out_insert_dict(data) #out sheet
    data['task_id'] = '0_wangxin_test_rollmicro_01'
    data.pop('_id')
    mongodb.out_insert_dict(data) # out sheet
    print("cooltrans")
